                ...
                cities.map((city, index) => (
                    (index % 3 === 0) ? (
                        <div key={index} className='flex flex-row mb-6'>
                            <a href={`/cities/${cities[index]}`} className='pr-6 drop-shadow-xl'>
                                <CityCard city={city} />
                            </a>
                            {cities[index + 1] && (
                                <a href={`/cities/${cities[index + 1]}`} className='pr-6 drop-shadow-xl'>
                                    <CityCard city={cities[index + 1]} />
                                </a>
                            )}
                            {cities[index + 2] && (
                                <a href={`/cities/${cities[index + 2]}`} className='drop-shadow-xl'>
